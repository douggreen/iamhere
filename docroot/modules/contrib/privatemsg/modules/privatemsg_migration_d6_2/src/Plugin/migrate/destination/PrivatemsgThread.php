...
class PrivatemsgThread extends DestinationBase implements ContainerFactoryPluginInterface {
  ...
  public function import(Row $row, array $old_destination_id_values = []) {
    ...
    foreach ($thread_members as $thread_member) {
      $tags_for_thread = [];
      foreach ($tags as $tag) {
        if ($thread_member === $tag['uid'] && $tag['tag'] === 'Favorites') {
          $favorites_tag = $this->privateMsgService->getFavoritesTag();
          $tags_for_thread[] = $favorites_tag->id();
        }

        if ($thread_member === $tag['uid'] && $tag['tag'] !== 'Inbox' && $tag['tag'] !== 'Favorites') {
